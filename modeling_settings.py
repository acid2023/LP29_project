...
def declare_keras_models(models_dict: dict, num_features: int) -> dict:
    class CustomKerasRegressor(KerasRegressor, BaseEstimator, RegressorMixin):
        ...
        def fit(self, X, y, **kwargs):
            ...
            self._modify_model_predict()
            return self.model_

        def _modify_model_predict(self):
            def custom_predict(self, X, **kwargs):
                if not tf.is_tensor(X):
                    X = tf.convert_to_tensor(X, dtype=tf.float32)
                return self.predict(X, **kwargs)

            self.model_.predict = types.MethodType(custom_predict, self.model_)
